...
def integers(sequence_of_sequences):
    ...
    for k in range (len(sequence_of_sequences)):
        tree=sequence_of_sequences[k]
        if type(tree) is list:
            for s in range (len(tree)):
                if type(tree[s]) is int:
                    seq=seq+[tree[s]]
        if type(tree) is int:
            seq=seq+[tree]
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'KRISTA'
    answer = big_letters([(1,2,3,4),
                          'Krista manche',  # K
                          (20, 'Krista Manche', 20),  # not a string
                          [],  # not a string
                          'krista',  #
                          ['MANCHE'],  # not a string
                          'kRista manche',  # R
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        ...
        if type(tree) is str:
            for s in range(len(tree)):
                if tree[s].isupper() is True:
                    string=string+tree[s]
